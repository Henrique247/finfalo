  ...
  app.post("/api/finbot", async (req, res) => {
    try {
      ...
      if (!userApiKey) {
        ...
        return res.json({
          text: `Olá! Sou o **FinBot** 🤖. Ligo-me diretamente às tuas finanças da **conta de ${typeLabel}** registada em nome de **${financialData.userName || "Utilizador"}**.\n\nComo posso ajudar-te no teu planeamento financeiro de hoje?`,
          isDemo: false
        });
      }
...
4. Mantenha as respostas relativamente curtas e focadas na pergunta, sem explicações genéricas longas.`;
      ...
      // Try configured keys sequentially if primary fails
      const apiKeysToTry = [
        financialData.customApiKey,
      ...
      // Loop through API keys and models
      for (const keyCandidate of apiKeysToTry) {
        if (success) break;
        try {
          const clientCandidate = new GoogleGenAI({
            apiKey: keyCandidate,
            httpOptions: {
              headers: {
                "User-Agent": "aistudio-build",
          ...
          for (const modelName of modelsToTry) {
            try {
              const response = await clientCandidate.models.generateContent({
                model: modelName,
                contents: contents,
                config: {
              ...
              console.warn(`[FinBot] Modelo ${modelName} falhou para chave:`, err?.message || err);
            }
          }
        } catch (keyErr: any) {
          console.warn("[FinBot] Falha com chave candidata:", keyErr?.message || keyErr);
        }
      }

      // If AI models fail due to high demand or network limits, generate a friendly local fallback reply
      if (!success || !replyText) {
        const netValue = incomes - expenses;
        replyText = `De momento estamos a enfrentar um problema técnico na ligação assistida.

Aqui está o resumo financeiro direto calculado pelo **FinBot**:
- **Saldo Atual:** ${balance.toLocaleString()} ${currency}
- **Receitas do Mês:** ${incomes.toLocaleString()} ${currency}
- **Despesas do Mês:** ${expenses.toLocaleString()} ${currency} (${netValue >= 0 ? 'Poupança positiva' : 'Atenção ao défice'})
- **Pontuação de Saúde Financeira:** ${healthScore}/100

💡 *Dica do FinBot:* Para manter a tua saúde financeira equilibrada, garante que reservaste pelo menos 20% das tuas receitas para a tua reserva de emergência. Tenta novamente em alguns instantes.`;
      }

      res.json({ text: replyText });
    } catch (error: any) {
      console.error("Erro na API FinBot:", error);
      // Even in top level catch, send fallback text without mentioning Gemini
      const fallbackText = `De momento estamos a enfrentar um problema. Por favor, tenta novamente em alguns instantes.`;
